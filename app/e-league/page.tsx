...
export default async function ELeaguePage() {
  ...
  return (
    <div className="flex flex-col w-full pb-24">
      {/* Hero */}
      <div className="relative bg-gradient-to-br from-purple-950/60 via-slate-950 to-slate-950 border-b border-kaf-border px-4 sm:px-8 py-12 overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-purple-500/10 border border-purple-500/30 text-purple-400 text-xs font-black uppercase tracking-widest mb-4">
            <Tv size={10} /> E-League
          </div>
          <h1 className="text-4xl sm:text-6xl font-display font-black text-white uppercase tracking-tight">
            KAF <span className="text-purple-400">E-League</span>
          </h1>
          ...
          {liveSeries.length > 0 && (
            <div className="mt-4 inline-flex items-center gap-2">
              ...
              <span className="text-red-400 text-sm font-bold">{liveSeries.length} Season{liveSeries.length > 1 ? 's' : ''} Live Now</span>
            </div>
          )}
        </div>
      </div>
...
function SeriesCard({ series, highlight = false }: { series: any; highlight?: boolean }) {
  ...
  return (
    <div className={`kaf-card rounded-2xl border overflow-hidden transition-all hover:scale-[1.01] ${
      ...
      <div className="p-4 space-y-2">
        ...
        {series.prize_pool && (
          <div className="text-xs text-brand-gold font-bold">🏆 {series.prize_pool}</div>
        )}
        {series.description && (
