...
export default async function PlayerDashboard() {
  ...
  const activeTournaments = myTournaments?.filter((t: any) =>
    t.tournaments?.status === 'active' || t.tournaments?.status === 'live' || t.tournaments?.status === 'in_progress' || t.tournaments?.status === 'registration_open'
  ) || []
            ...
            <div className="kaf-card kaf-cut border border-kaf-border p-6 flex flex-col justify-center items-center text-center">
              ...
              {clanMembership?.clans ? (
                <>
                  ...
                  <Link href={`/clans/${clanMembership.clan_id}`} className="text-xs font-bold text-slate-400 uppercase tracking-widest hover:text-white transition">
                    Open Clan HQ
                  </Link>
                </>
              ) : (
