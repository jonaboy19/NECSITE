...
export default function LeftSidebar() {
  ...
  return (
    <aside className="sticky top-0 hidden h-screen w-64 flex-col border-r border-kaf-border bg-kaf-panel shadow-panel no-scrollbar lg:flex overflow-hidden">
      ...
      <div className="flex flex-col h-full overflow-y-auto no-scrollbar p-4 gap-4">
        ...
        {/* Logo */}
        <Link href="/" className="flex items-center gap-3 px-2 py-1 group shrink-0">
          <div className="relative w-10 h-10 rounded-xl overflow-hidden bg-brand-cyan/10 border border-brand-cyan/20 flex items-center justify-center shrink-0 group-hover:border-brand-cyan/50 transition-colors">
            <Image
              src="/kaf-logo.png"
        ...
        <button
          ...
          className="flex items-center gap-2.5 w-full px-3 py-2.5 rounded-xl bg-kaf-card border border-kaf-border text-slate-500 hover:text-slate-300 hover:border-kaf-border-strong text-xs transition-all group shrink-0"
        >
          <Search size={13} className="shrink-0" />
          <span className="flex-1 text-left">Search...</span>
          <kbd className="text-[9px] border border-kaf-border rounded-md px-1.5 py-0.5 font-mono hidden group-hover:block">
            Ctrl K
          </kbd>
        </button>
        ...
        <nav className="flex flex-1 flex-col gap-5 min-h-0">
                    ...
                    {section.items.map((item) => {
                      ...
                      return (
                        <Link
                          key={item.name}
                          href={item.href}
                          className={`relative flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-bold transition-all duration-150
                            ${active
          ...
          {/* Info section */}
          <div>
            <p className="mb-1.5 px-2 text-[10px] font-black uppercase tracking-[0.18em] text-slate-600">Info</p>
            <div className="flex flex-col gap-0.5">
              {INFO_ITEMS.map(item => {
                const active = isActive(item.href)
                const iconColor = active ? '#22c55e' : item.accent || '#64748b'
        ...
        <div className="shrink-0 pt-3 border-t border-kaf-border">
          {userProfile ? (
            <Link
              ...
              className="group flex items-center gap-3 rounded-xl p-3 border border-kaf-border hover:border-brand-cyan/30 hover:bg-kaf-card transition-all"
            >
